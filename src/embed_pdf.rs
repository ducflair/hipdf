...
use lopdf::{Document, Object, Dictionary, dictionary, content::{Operation, Content}, Stream};
...
impl PdfEmbedder {
    ...
    pub fn load_pdf(&mut self, path: impl AsRef<Path>) -> Result<String> {
        ...
        let source_doc = Document::load(path.as_ref())
            .map_err(|e| Error::new(ErrorKind::InvalidData, format!("Failed to load PDF: {}", e)))?;
        let info = self.extract_pdf_info(&source_doc)?;

        self.loaded_pdfs.insert(path_str.clone(), (source_doc, info));
        Ok(path_str)
    }
    ...
    pub fn load_pdf_from_bytes(&mut self, bytes: &[u8], identifier: &str) -> Result<String> {
        ...
        let source_doc = Document::load_from(bytes)
            .map_err(|e| Error::new(ErrorKind::InvalidData, format!("Failed to load PDF from bytes: {}", e)))?;
        let info = self.extract_pdf_info(&source_doc)?;

        self.loaded_pdfs.insert(identifier.to_string(), (source_doc, info));
    ...
    pub fn embed_pdf(
        ...
        // Get source document
        let (source_doc, info) = self.loaded_pdfs.get(source_identifier)
        ...
        if let Some((clip_x, clip_y, clip_w, clip_h)) = options.clip_bounds {
            ...
            all_operations.push(Operation::new("re", vec![
                clip_x.into(), clip_y.into(), clip_w.into(), clip_h.into()
            ]));
    ...
    fn import_page_as_xobject(
        ...
        let page_obj = source_doc.get_object(*page_id)
            .map_err(|e| Error::new(ErrorKind::InvalidData, format!("Failed to get page object: {}", e)))?;
        
        let page_dict = page_obj.as_dict()
            .map_err(|e| Error::new(ErrorKind::InvalidData, format!("Page object is not a dictionary: {}", e)))?;
    ...
    fn get_page_content_stream(&self, source_doc: &Document, page_dict: &Dictionary) -> Result<Vec<u8>> {
    ...
    fn copy_object_to_target(&self, source_doc: &Document, target_doc: &mut Document, obj: &Object) -> Result<Object> {
        match obj {
            Object::Reference(ref_id) => {
            ...
            Object::Stream(stream) => {
                let new_dict = if let Object::Dictionary(dict) = self.copy_object_to_target(source_doc, target_doc, &Object::Dictionary(stream.dict.clone()))? {
                    dict
                } else {
                    Dictionary::new()
                };
            ...
            match media_box_obj {
                Object::Reference(ref_id) => {
                    if let Ok(actual_obj) = source_doc.get_object(*ref_id) {
                        ...
                        // Default A4 size
                        Ok(Object::Array(vec![0.into(), 0.into(), 595.into(), 842.into()]))
                    }
                },
    ...
    fn place_xobject(
        ...
        // Draw the XObject
        operations.push(Operation::new("Do", vec![
            Object::Name(xobject_name.as_bytes().to_vec())
        ]));
        ...
        for (idx, &page_num) in pages.iter().enumerate() {
            ...
            let (x, y) = match options.layout {
                ...
                MultiPageLayout::Vertical { gap } => {
                    let total_height: f32 = (0..idx).map(|i| {
                ...
                MultiPageLayout::Horizontal { gap } => {
                    let total_width: f32 = (0..idx).map(|i| {
                        let (w, _) = info.page_dimensions[pages[i]];
                        w * scale_x + gap
                    }).sum();
                    (base_x + total_width, base_y)
                },
                MultiPageLayout::Grid { columns, gap_x, gap_y, fill_order } => {
