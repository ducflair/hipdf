...
use lopdf::{dictionary, Document, Object, ObjectId, Dictionary, content::{Content, Operation}, Stream};
...
pub enum CustomPattern {
    ...
    Parametric(Arc<dyn Fn(f32, f32, &PatternParams) -> Vec<Operation> + Send + Sync>, PatternParams),
...
impl std::fmt::Debug for CustomPattern {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            CustomPattern::Parametric(_, params) => f.debug_tuple("Parametric").field(&"<function>").field(params).finish(),
            CustomPattern::Procedural(proc) => f.debug_tuple("Procedural").field(proc).finish(),
            CustomPattern::Composite(elements) => f.debug_tuple("Composite").field(elements).finish(),
        }
    }
}
...
impl CustomPatternBuilder {
    ...
    pub fn move_to(&mut self, x: f32, y: f32) -> &mut Self {
        self.current_path.push(("m".to_string(), vec![x.into(), y.into()]));
        self
    }
    
    pub fn line_to(&mut self, x: f32, y: f32) -> &mut Self {
        self.current_path.push(("l".to_string(), vec![x.into(), y.into()]));
        self
    }
    
    pub fn curve_to(&mut self, cx1: f32, cy1: f32, cx2: f32, cy2: f32, x: f32, y: f32) -> &mut Self {
        self.current_path.push(("c".to_string(), vec![
            cx1.into(), cy1.into(), cx2.into(), cy2.into(), x.into(), y.into()
        ]));
        self
    }
    ...
    pub fn set_line_width(&mut self, width: f32) -> &mut Self {
        self.operations.push(Operation::new("w", vec![width.into()]));
        self
    }
    
    pub fn set_stroke_color(&mut self, r: f32, g: f32, b: f32) -> &mut Self {
        self.operations.push(Operation::new("RG", vec![r.into(), g.into(), b.into()]));
        self
    }
    ...
    pub fn set_dash_pattern(&mut self, pattern: Vec<f32>, phase: f32) -> &mut Self {
        ...
        self.operations.push(Operation::new("d", vec![
            Object::Array(array),
            phase.into()
        ]));
        self
    ...
    pub fn rectangle(&mut self, x: f32, y: f32, width: f32, height: f32) -> &mut Self {
        self.operations.push(Operation::new("re", vec![x.into(), y.into(), width.into(), height.into()]));
        self
    }
...
impl HatchingManager {
    ...
    fn calculate_pattern_bounds(&self, config: &HatchConfig) -> (f32, f32) {
        ...
        match config.style {
            ...
            HatchStyle::Circles | HatchStyle::ConcentricCircles => (base_size * 2.0, base_size * 2.0),
    ...
    fn generate_pattern_operations(
        ...
        if let Some((r, g, b)) = config.background {
            ...
            ops.push(Operation::new("re", vec![0.into(), 0.into(), width.into(), height.into()]));
    ...
    fn checkerboard_ops(&self, ops: &mut Vec<Operation>, width: f32, height: f32) {
        ops.push(Operation::new("re", vec![0.into(), 0.into(), (width/2.0).into(), (height/2.0).into()]));
        ops.push(Operation::new("re", vec![(width/2.0).into(), (height/2.0).into(), (width/2.0).into(), (height/2.0).into()]));
        ops.push(Operation::new("f", vec![]));
    }
    ...
    fn brick_ops(&self, ops: &mut Vec<Operation>, width: f32, height: f32) {
        ...
        ops.push(Operation::new("l", vec![(width/4.0).into(), (height/2.0).into()]));
        ops.push(Operation::new("S", vec![]));
        
        ops.push(Operation::new("m", vec![(width*3.0/4.0).into(), (height/2.0).into()]));
        ops.push(Operation::new("l", vec![(width*3.0/4.0).into(), height.into()]));
        ops.push(Operation::new("S", vec![]));
    }
    ...
    fn wave_ops(&self, ops: &mut Vec<Operation>, width: f32, height: f32) {
        ...
        ops.push(Operation::new("c", vec![
            (width/4.0).into(), 0.into(),
            (width*3.0/4.0).into(), height.into(),
            width.into(), (height/2.0).into()
        ]));
        ops.push(Operation::new("S", vec![]));
    }
    ...
    fn zigzag_ops(&self, ops: &mut Vec<Operation>, width: f32, height: f32) {
        ...
        ops.push(Operation::new("l", vec![(width/2.0).into(), height.into()]));
        ops.push(Operation::new("l", vec![(width*3.0/4.0).into(), 0.into()]));
        ops.push(Operation::new("l", vec![width.into(), (height/2.0).into()]));
        ops.push(Operation::new("S", vec![]));
    }
    ...
    fn wood_grain_ops(&self, ops: &mut Vec<Operation>, width: f32, height: f32) {
        for i in 0..3 {
            ...
            ops.push(Operation::new("c", vec![
                (width*0.2).into(), (y - height*0.1).into(),
                (width*0.8).into(), (y + height*0.1).into(),
                width.into(), y.into()
            ]));
            ops.push(Operation::new("S", vec![]));
        }
    ...
    fn circle_at(&self, ops: &mut Vec<Operation>, cx: f32, cy: f32, r: f32) {
        ...
        ops.push(Operation::new("m", vec![(cx + r).into(), cy.into()]));
        ops.push(Operation::new("c", vec![
            (cx + r).into(), (cy + k * r).into(),
            (cx + k * r).into(), (cy + r).into(),
            cx.into(), (cy + r).into()
        ]));
        ops.push(Operation::new("c", vec![
            (cx - k * r).into(), (cy + r).into(),
    ...
    fn arc_at(&self, ops: &mut Vec<Operation>, cx: f32, cy: f32, r: f32, start_angle: f32, end_angle: f32) {
        ...
        let control_distance = r * 0.552284749831;
    ...
    fn generate_procedural_pattern(&self, proc: &ProceduralPattern, width: f32, height: f32) -> Vec<Operation> {
        let mut ops = Vec::new();
        ...
        for i in 0..proc.resolution {
            for j in 0..proc.resolution {
                let x = i as f32 * step;
                let y = j as f32 * step;
                let t = (i as f32 / proc.resolution as f32 + j as f32 / proc.resolution as f32) / 2.0;
                
                if (proc.sampler)(x, y, t) {
                    if proc.fill {
                        ops.push(Operation::new("re", vec![x.into(), y.into(), step.into(), step.into()]));
                        ops.push(Operation::new("f", vec![]));
                    } else {
                        let cx = x + step / 2.0;
                        let cy = y + step / 2.0;
...
impl PatternedShapeBuilder {
    ...
    pub fn rectangle(&mut self, x: f32, y: f32, width: f32, height: f32, pattern_name: &str) -> &mut Self {
        self.operations.push(PatternOperations::set_pattern_fill_colorspace());
        self.operations.push(PatternOperations::set_fill_pattern(pattern_name));
        self.operations.push(Operation::new("re", vec![x.into(), y.into(), width.into(), height.into()]));
        self.operations.push(Operation::new("f", vec![]));
        self
    }
    ...
    pub fn circle(&mut self, cx: f32, cy: f32, r: f32, pattern_name: &str) -> &mut Self {
        ...
        let k = 0.552284749831;
        self.operations.push(Operation::new("m", vec![(cx + r).into(), cy.into()]));
        self.operations.push(Operation::new("c", vec![
            (cx + r).into(), (cy + k * r).into(),
            (cx + k * r).into(), (cy + r).into(),
            cx.into(), (cy + r).into()
        ]));
    ...
    pub fn triangle(&mut self, x1: f32, y1: f32, x2: f32, y2: f32, x3: f32, y3: f32, pattern_name: &str) -> &mut Self {
        self.operations.push(PatternOperations::set_pattern_fill_colorspace());
        self.operations.push(PatternOperations::set_fill_pattern(pattern_name));
        self.operations.push(Operation::new("m", vec![x1.into(), y1.into()]));
        self.operations.push(Operation::new("l", vec![x2.into(), y2.into()]));
        self.operations.push(Operation::new("l", vec![x3.into(), y3.into()]));
