...
use hipdf::hatching::{HatchingManager, HatchConfig, HatchStyle, PatternedShapeBuilder, CustomPattern, ProceduralPattern, Transform};
...
fn test_hatching_patterns_showcase() -> Result<()> {
    ...
    text_ops.push(lopdf::content::Operation::new("Tf", vec![
        Object::Name(b"F1".to_vec()),
        14.into()
    ]));
    text_ops.push(lopdf::content::Operation::new("Td", vec![200.into(), 800.into()]));
    text_ops.push(lopdf::content::Operation::new("Tj", vec![
        Object::string_literal("20 Different Hatching Patterns")
    ]));
    ...
    for (index, (name, _config)) in patterns.iter().enumerate() {
        ...
        text_ops.push(lopdf::content::Operation::new("Td", vec![
            x.into(),
            (y - 15.0).into()
        ]));
        ...
        text_ops.push(lopdf::content::Operation::new("Tj", vec![
            Object::string_literal(label.as_bytes().to_vec())
        ]));
        text_ops.push(lopdf::content::Operation::new("ET", vec![]));
    }
    ...
    text_ops.push(lopdf::content::Operation::new("Td", vec![50.into(), 100.into()]));
    text_ops.push(lopdf::content::Operation::new("Tj", vec![
        Object::string_literal("Demo: Different shapes with various patterns")
    ]));
    text_ops.push(lopdf::content::Operation::new("ET", vec![]));
...
fn test_custom_patterns_showcase() -> Result<()> {
    ...
    let (pattern_id, pattern_name) = hatching_manager.create_custom_pattern(
        &mut doc, 20.0, 20.0,
        |builder| {
            builder
                .set_fill_color(0.8, 0.2, 0.2)
                .circle(5.0, 5.0, 2.0).fill()
                .circle(15.0, 8.0, 1.5).fill()
                .circle(10.0, 15.0, 2.5).fill()
        }
    );
    hatching_manager.add_pattern_to_resources(&mut resources, &pattern_name, pattern_id);
    ...
    let (pattern_id, pattern_name) = hatching_manager.create_custom_pattern(
        ...
        |builder| {
            builder
                ...
                .set_line_width(1.0)
                // Outer square
                .rectangle(5.0, 5.0, 30.0, 30.0).stroke()
                ...
                .rectangle(-10.0, -10.0, 20.0, 20.0).stroke()
                .pop_transform()
                // Center circle
                .circle(20.0, 20.0, 5.0).stroke()
        }
    );
    hatching_manager.add_pattern_to_resources(&mut resources, &pattern_name, pattern_id);
    ...
    let sierpinski = HatchConfig::new(HatchStyle::Custom(
        CustomPattern::Procedural(ProceduralPattern {
    ...
    text_ops.push(lopdf::content::Operation::new("0 g", vec![])); // Set solid black fill for text
    text_ops.push(lopdf::content::Operation::new("BT", vec![]));
    text_ops.push(lopdf::content::Operation::new("Tf", vec![Object::Name(b"F1".to_vec()), 16.into()]));
    text_ops.push(lopdf::content::Operation::new("Td", vec![150.into(), 800.into()]));
    text_ops.push(lopdf::content::Operation::new("Tj", vec![Object::string_literal("Custom Pattern Examples")]));
    text_ops.push(lopdf::content::Operation::new("ET", vec![]));
    ...
    for (index, (key, label)) in patterns.iter().enumerate() {
        ...
        text_ops.push(lopdf::content::Operation::new("0 g", vec![])); // Set solid black fill for text
        text_ops.push(lopdf::content::Operation::new("BT", vec![]));
        text_ops.push(lopdf::content::Operation::new("Tf", vec![Object::Name(b"F1".to_vec()), 10.into()]));
        text_ops.push(lopdf::content::Operation::new("Td", vec![x.into(), (y - 15.0).into()]));
        text_ops.push(lopdf::content::Operation::new("Tj", vec![Object::string_literal(label.as_bytes().to_vec())]));
        text_ops.push(lopdf::content::Operation::new("ET", vec![]));
    }
    ...
    let pages_dict = doc.get_object_mut(pages_id).and_then(Object::as_dict_mut).unwrap();
    pages_dict.set("Kids", vec![Object::Reference(page_id)]);
    ...
    let output_path = std::path::Path::new("tests/outputs/hatching_custom_patterns_integration_test.pdf");
    let absolute_path = std::env::current_dir()?.join(output_path);
    doc.save(&absolute_path)?;
