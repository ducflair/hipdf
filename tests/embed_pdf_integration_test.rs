...
use hipdf::embed_pdf::{
    PdfEmbedder, EmbedOptions, MultiPageLayout, GridFillOrder,
    PageRange, CustomLayoutStrategy
};
use lopdf::{dictionary, Document, Object, content::Content, Stream, Dictionary};
...
fn create_page_with_title(doc: &mut Document, title: &str) -> (Object, Vec<lopdf::content::Operation>, HashMap<String, Object>) {
    // Add font for labels
...
fn test_vertical_layouts() {
    ...
    let (font_id, mut page_ops, mut all_xobjects) = create_page_with_title(&mut doc, "Vertical Layout Examples");
    ...
    add_section_label(&mut page_ops, 50.0, 750.0, "1. Vertical stack - First 3 pages (small gap)");
    ...
    let result = embedder.embed_pdf(&mut doc, &arxiv_pdf, &vertical_small_gap).unwrap();
    page_ops.extend(result.operations);
    all_xobjects.extend(result.xobject_resources);

    // Example 2: Vertical stack with large gap
    add_section_label(&mut page_ops, 200.0, 750.0, "2. Vertical stack - Pages 2-5 (large gap)");
    ...
    let result = embedder.embed_pdf(&mut doc, &arxiv_pdf, &vertical_large_gap).unwrap();
    page_ops.extend(result.operations);
    all_xobjects.extend(result.xobject_resources);
    ...
    let result = embedder.embed_pdf(&mut doc, &arxiv_pdf, &vertical_varied).unwrap();
    page_ops.extend(result.operations);
    all_xobjects.extend(result.xobject_resources);
...
fn test_horizontal_layouts() {
    ...
    let (font_id, mut page_ops, mut all_xobjects) = create_page_with_title(&mut doc, "Horizontal Layout Examples");

    let mut embedder = PdfEmbedder::new();
    let arxiv_pdf = embedder.load_pdf("tests/assets/2412.07377v3.pdf").unwrap();

    // Example 1: Horizontal spread - small pages
    add_section_label(&mut page_ops, 50.0, 700.0, "1. Horizontal spread - First 4 pages (small)");
    ...
    let result = embedder.embed_pdf(&mut doc, &arxiv_pdf, &horizontal_small).unwrap();
    page_ops.extend(result.operations);
    all_xobjects.extend(result.xobject_resources);

    // Example 2: Horizontal spread - medium pages with tight spacing
    add_section_label(&mut page_ops, 50.0, 550.0, "2. Horizontal spread - Pages 2-4 (medium, tight)");
    ...
    let result = embedder.embed_pdf(&mut doc, &arxiv_pdf, &horizontal_medium).unwrap();
    page_ops.extend(result.operations);
    all_xobjects.extend(result.xobject_resources);

    // Example 3: Horizontal spread - varying widths
    add_section_label(&mut page_ops, 50.0, 370.0, "3. Horizontal spread - Custom selection");
    ...
    let result = embedder.embed_pdf(&mut doc, &arxiv_pdf, &horizontal_custom).unwrap();
    page_ops.extend(result.operations);
    all_xobjects.extend(result.xobject_resources);
...
fn test_grid_layouts() {
    ...
    let (font_id, mut page_ops, mut all_xobjects) = create_page_with_title(&mut doc, "Grid Layout Examples");
    ...
    let result = embedder.embed_pdf(&mut doc, &arxiv_pdf, &grid_2x3_row).unwrap();
    page_ops.extend(result.operations);
    all_xobjects.extend(result.xobject_resources);

    // Example 2: 3x2 Grid (Column First)
    add_section_label(&mut page_ops, 300.0, 700.0, "2. 3x2 Grid - Column First Fill");
    ...
    let result = embedder.embed_pdf(&mut doc, &arxiv_pdf, &grid_3x2_col).unwrap();
    page_ops.extend(result.operations);
    all_xobjects.extend(result.xobject_resources);
    ...
    let result = embedder.embed_pdf(&mut doc, &arxiv_pdf, &grid_4x4_thumb).unwrap();
    page_ops.extend(result.operations);
    all_xobjects.extend(result.xobject_resources);
...
fn test_scaling_and_sizing() {
    ...
    let (font_id, mut page_ops, mut all_xobjects) = create_page_with_title(&mut doc, "Scaling and Sizing Examples");
    ...
    let result = embedder.embed_pdf(&mut doc, &arxiv_pdf, &scale_tiny).unwrap();
    page_ops.extend(result.operations);
    all_xobjects.extend(result.xobject_resources);
    ...
    let result = embedder.embed_pdf(&mut doc, &arxiv_pdf, &scale_small).unwrap();
    page_ops.extend(result.operations);
    all_xobjects.extend(result.xobject_resources);
    ...
    let result = embedder.embed_pdf(&mut doc, &arxiv_pdf, &scale_medium).unwrap();
    page_ops.extend(result.operations);
    all_xobjects.extend(result.xobject_resources);

    // Example 2: Non-uniform scaling
    add_section_label(&mut page_ops, 50.0, 450.0, "4. Non-uniform: Wide (0.3x, 0.1x)");
    let scale_wide = EmbedOptions::new()
        .at_position(50.0, 400.0)
        .with_scale_xy(0.3, 0.1)
        .with_layout(MultiPageLayout::FirstPageOnly);

    let result = embedder.embed_pdf(&mut doc, &arxiv_pdf, &scale_wide).unwrap();
    page_ops.extend(result.operations);
    all_xobjects.extend(result.xobject_resources);

    add_section_label(&mut page_ops, 300.0, 450.0, "5. Non-uniform: Tall (0.1x, 0.3x)");
    let scale_tall = EmbedOptions::new()
        .at_position(300.0, 400.0)
        .with_scale_xy(0.1, 0.3)
        .with_layout(MultiPageLayout::FirstPageOnly);

    let result = embedder.embed_pdf(&mut doc, &arxiv_pdf, &scale_tall).unwrap();
    page_ops.extend(result.operations);
    all_xobjects.extend(result.xobject_resources);
    ...
    let result = embedder.embed_pdf(&mut doc, &arxiv_pdf, &max_size_1).unwrap();
    page_ops.extend(result.operations);
    all_xobjects.extend(result.xobject_resources);
    ...
    let result = embedder.embed_pdf(&mut doc, &arxiv_pdf, &max_size_2).unwrap();
    page_ops.extend(result.operations);
    all_xobjects.extend(result.xobject_resources);
    ...
    let result = embedder.embed_pdf(&mut doc, &arxiv_pdf, &max_size_square).unwrap();
    page_ops.extend(result.operations);
    all_xobjects.extend(result.xobject_resources);
...
fn test_custom_layouts() {
    ...
    let (font_id, mut page_ops, mut all_xobjects) = create_page_with_title(&mut doc, "Custom Layout Examples");
    ...
    let diagonal_strategy = CustomLayoutStrategy {
        position_fn: |idx, _w, _h| {
            (idx as f32 * 50.0, -(idx as f32 * 50.0))
        },
        scale_fn: |idx| {
    ...
    let result = embedder.embed_pdf(&mut doc, &arxiv_pdf, &diagonal_cascade).unwrap();
    page_ops.extend(result.operations);
    all_xobjects.extend(result.xobject_resources);
    ...
    let result = embedder.embed_pdf(&mut doc, &arxiv_pdf, &decreasing).unwrap();
    page_ops.extend(result.operations);
    all_xobjects.extend(result.xobject_resources);
...
fn test_page_ranges() {
    ...
    let (font_id, mut page_ops, mut all_xobjects) = create_page_with_title(&mut doc, "Page Range Selection Examples");
    ...
    let result = embedder.embed_pdf(&mut doc, &arxiv_pdf, &single_page).unwrap();
    page_ops.extend(result.operations);
    all_xobjects.extend(result.xobject_resources);
    ...
    let result = embedder.embed_pdf(&mut doc, &arxiv_pdf, &all_pages).unwrap();
    page_ops.extend(result.operations);
    all_xobjects.extend(result.xobject_resources);
...
fn test_rotation_and_transformations() {
    ...
    let (font_id, mut page_ops, mut all_xobjects) = create_page_with_title(&mut doc, "Rotation and Transformation Examples");
    ...
    let result = embedder.embed_pdf(&mut doc, &arxiv_pdf, &rotated_grid).unwrap();
    page_ops.extend(result.operations);
    all_xobjects.extend(result.xobject_resources);
...
fn test_mixed_pdfs() {
    ...
    let (font_id, mut page_ops, mut all_xobjects) = create_page_with_title(&mut doc, "Mixed PDF Embedding");
    ...
    for row in 0..3 {
        let mut x_pos = 50.0;
        for col in 0..4 {
            let pdf_to_use = if (row + col) % 2 == 0 { &arxiv_pdf } else { &lines_pdf };
    ...
    let result = embedder.embed_pdf(&mut doc, &arxiv_pdf, &comparison_left).unwrap();
    page_ops.extend(result.operations);
    all_xobjects.extend(result.xobject_resources);
    ...
    let result = embedder.embed_pdf(&mut doc, &lines_pdf, &comparison_right).unwrap();
    page_ops.extend(result.operations);
    all_xobjects.extend(result.xobject_resources);
...
fn test_clipping_and_bounds() {
    ...
    let (font_id, mut page_ops, mut all_xobjects) = create_page_with_title(&mut doc, "Clipping and Bounds Examples");
    ...
    let result = embedder.embed_pdf(&mut doc, &arxiv_pdf, &clipped_small).unwrap();
    page_ops.extend(result.operations);
    all_xobjects.extend(result.xobject_resources);
    ...
    let result = embedder.embed_pdf(&mut doc, &arxiv_pdf, &clipped_grid).unwrap();
    page_ops.extend(result.operations);
    all_xobjects.extend(result.xobject_resources);
    ...
    let result = embedder.embed_pdf(&mut doc, &arxiv_pdf, &preserve_aspect).unwrap();
    page_ops.extend(result.operations);
    all_xobjects.extend(result.xobject_resources);
    ...
    let result = embedder.embed_pdf(&mut doc, &arxiv_pdf, &stretched).unwrap();
    page_ops.extend(result.operations);
    all_xobjects.extend(result.xobject_resources);
...
fn test_comprehensive_showcase() {
    ...
    let (font_id, mut page_ops, mut all_xobjects) = create_page_with_title(&mut doc, "PDF Embedding - Comprehensive Showcase");

    let mut embedder = PdfEmbedder::new();
    let arxiv_pdf = embedder.load_pdf("tests/assets/2412.07377v3.pdf").unwrap();

    // Row 1: Different scales
    add_section_label(&mut page_ops, 50.0, 720.0, "Scales: 0.05, 0.1, 0.15, 0.2, 0.25");
    let scales = vec![0.05, 0.1, 0.15, 0.2, 0.25];
    ...
    let grid_configs = vec![(2, 2), (3, 2), (4, 2)];
    ...
    add_section_label(&mut page_ops, 50.0, 430.0, "Page Selections: All Even, All Odd, First 3, Last 3");
    ...
    let result = embedder.embed_pdf(&mut doc, &arxiv_pdf, &even_pages).unwrap();
    page_ops.extend(result.operations);
    all_xobjects.extend(result.xobject_resources);
    ...
    let result = embedder.embed_pdf(&mut doc, &arxiv_pdf, &odd_pages).unwrap();
    page_ops.extend(result.operations);
    all_xobjects.extend(result.xobject_resources);

    // Row 4: Rotations
    add_section_label(&mut page_ops, 50.0, 250.0, "Rotation angles: 0°, 30°, 60°, 90°, 120°, 150°");
    let angles = vec![0.0, 30.0, 60.0, 90.0, 120.0, 150.0];
